# Go Coding Style And Naming Conventions

## Status

- Version: 0.1.0
- State: draft; manual approval required
- Kind: development guide
- Source: `docs/library-curation/drafts/guides/development/go-coding-style.md`

## Verbatim Candidate

> # Go Coding Style And Naming Conventions
>
> Output type: development guide. Keep this source intact for initial dogfooding;
> its split among specifications, guides, and skills remains open.
>
> Source: [`promisegrid_wire-lab_refs_heads_main_AGENTS.md`](../../reference/source-guides/promisegrid_wire-lab_refs_heads_main_AGENTS.md#coding-style--naming-conventions)
>
> > - Use object-oriented design with structs and methods; avoid large functions and global state.
> > - Follow generally accepted object oriented design patterns.
> > - Keep Go code `gofmt`-clean; package names should be short and lower-case.
> > - Prefer focused edits over broad refactors unless required.
> > - Add and maintain explanatory comments for non-obvious logic.
> > - Use `git mv` for file moves/renames to preserve history.
